...
# Button properties
button_size = 30
restart_button = {'x': 50, 'y': 30, 'size': button_size, 'color': [0.0, 0.8, 0.8]}  # Teal
play_pause_button = {'x': window_width // 2, 'y': 30, 'size': button_size, 'color': [1.0, 0.7, 0.0]}  # Amber
exit_button = {'x': window_width - 50, 'y': 30, 'size': button_size, 'color': [1.0, 0.0, 0.0]}  # Red
...
def draw_catcher(x, y, width, height, r, g, b):
    ...
    half_width = width // 2
    draw_line_midpoint(x - half_width, y, x, y - height, r, g, b)  # Bottom left
    draw_line_midpoint(x, y - height, x + half_width, y, r, g, b)  # Bottom right
    draw_line_midpoint(x + half_width, y, x + half_width - 10, y + 10, r, g, b)  # Top right
    draw_line_midpoint(x + half_width - 10, y + 10, x - half_width + 10, y + 10, r, g, b)  # Top
    draw_line_midpoint(x - half_width + 10, y + 10, x - half_width, y, r, g, b)  # Top left
...
def draw_restart_button(x, y, size, r, g, b):
    ...
    draw_line_midpoint(x - size//4, y - size//4, x - size//4, y + size//4, r, g, b)

def draw_play_button(x, y, size, r, g, b):
    # Draw a play button (triangle)
    draw_line_midpoint(x - size//3, y - size//3, x - size//3, y + size//3, r, g, b)
...
def draw_exit_button(x, y, size, r, g, b):
    ...
    draw_line_midpoint(x - size//2, y - size//2, x + size//2, y + size//2, r, g, b)
    draw_line_midpoint(x - size//2, y + size//2, x + size//2, y - size//2, r, g, b)
...
def display():
    ...
    if game_state != GAME_OVER:
        draw_diamond(diamond_x, diamond_y, diamond_size, diamond_color[0], diamond_color[1], diamond_color[2])
    
    # Draw catcher
    if game_state == GAME_OVER:
        draw_catcher(catcher_x, catcher_y, catcher_width, catcher_height, 1.0, 0.0, 0.0)  # Red when game over
    else:
        draw_catcher(catcher_x, catcher_y, catcher_width, catcher_height, catcher_color[0], catcher_color[1], catcher_color[2])
